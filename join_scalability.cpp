...
void GenDatabase(duckdb::Connection &con) {
	// database setting
	const size_t probing_size = 5e5;
	const size_t building_size = 5e4;
...
int main() {
	...
	for (size_t i = 64; i > 63; i -= 8) {
		for (size_t j = 64; j > 63; j -= 8) {
			double n_building = i;
			double n_combine = i;
			double n_probing = j;

			scheduler.SetThreadSetting(n_building, VecStr {""}, VecStr {"PIECEWISE_MERGE_JOIN"}, true);
	...
	Log2File(the_stats, "sort_merge_join_scalability.log");
